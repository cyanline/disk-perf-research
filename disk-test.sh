...
do
  ...
  count=`expr $size / $i`
  actual_size=`expr $i \* $count` # to handle cases where the bs is not a multiple of the size
  start_time=`date +%s` # get start time
  dd if=$DISK of=$OUTPUT bs=$i count=$count  >>$1 2>&1 
  ...
  printf "DS=%d,BS=%d,TS=%d,TP=%d\n" $actual_size $i $total_time $throughput  >> $1 # data line
  echo "---" >> $1
done
